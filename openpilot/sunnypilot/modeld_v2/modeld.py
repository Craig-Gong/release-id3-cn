...
def load_models_with_fallback(*, chestnut, load_big, load_small, params, update_loading_progress, hold_loading=False):
  ...
  if chestnut:
    try:
      ...
      # not leave loading into a false fallback flash.
      keep_loading = True

  if not keep_loading and not hold_loading:
  ...
  if model is None:
    ...
    if hold_loading:
      keep_loading = True
      params.put_bool("ChestnutLoading", True, block=True)
  elif chestnut:
    try:
      small_model = load_small()
    except Exception:
