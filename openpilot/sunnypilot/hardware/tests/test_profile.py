...
from openpilot.sunnypilot.hardware.profile import (
  HardwareProfile, allows_automatic_power_down, get_hardware_profile, has_amplifier, has_driver_camera, has_microphone,
  power_down_requested, model_compile_cpu, resolve_internal_panda_type,
)
...
def test_apply_c3xl_ife_runtime_opt_in_only(tmp_path, monkeypatch) -> None:
  from openpilot.sunnypilot.hardware import profile
  ...
  env: dict[str, str] = {}
  assert not profile.apply_c3xl_ife_runtime(env, profile=HardwareProfile.C3XL)
  assert "C3XL_IFE_ROAD_SIZE" not in env
  assert not profile_file.exists()

  env_on = {"C3XL_IFE_ROAD_SIZE": "1344x760", "C3XL_CTMV2_INPUT_RESIZE": "1"}
  assert profile.apply_c3xl_ife_runtime(env_on, profile=HardwareProfile.C3XL)
  assert profile_file.read_text() == "c3xl"
  assert "C3XL_CTMV2_INPUT_RESIZE" not in env_on
...
def test_microphone_capability_is_profile_scoped() -> None:
