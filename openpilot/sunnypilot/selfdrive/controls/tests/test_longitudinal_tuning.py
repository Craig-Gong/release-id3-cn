...
def test_default_profile_shifts_obstacle_for_codegen_stop_distance():
  params = FakeParams()
  tuning = backend_values(params, get_backend(BackendId.OFFICIAL))

  assert tuning.as_dict() == DEFAULT_VALUES
  # Default stop_distance=3.5 m; acados is baked at 4 m → shift obstacle +0.5 m.
  assert adjusted_obstacle(42.0, 0.0, 0.0, tuning, 1.45) == 42.5


def test_codegen_matched_tuning_returns_the_original_obstacle():
  raw = np.array([42.0, 42.00000000000001, 1.0e8], dtype=np.float64)
  tuning = LongitudinalTuning(stop_distance=4.0)

  adjusted = adjusted_obstacle(raw, 15.0, 15.0, tuning, 1.45)
...
def test_comfort_brake_and_stop_distance_adjust_the_solver_obstacle():
  ...
  # The compiled solver keeps CODEGEN comfort 2.5 m/s² and stop 4 m.
