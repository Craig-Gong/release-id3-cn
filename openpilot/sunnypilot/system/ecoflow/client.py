"""EcoFlow Delta 3 cloud client (App-login MQTT) — local verification only.
...
D361 JSON modes (mpptCar / setDp3) kept as fallbacks; they TIMEOUT on P231.

Do not commit secrets. Keep under demos/ until verified, then port to C3XL.
"""
...
def _load_local_env_files() -> None:
  ...
  # Parent-repo demos path when running from IQ.Pilot checkout
  try:
    demos = Path(__file__).resolve().parents[3] / "demos" / "ecoflow" / "ecoflow.env"
    candidates.append(demos)
  except IndexError:
    pass
