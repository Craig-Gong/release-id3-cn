...
from iqpilot.selfdrive.controls.lib.traffic_stop_offset import TrafficStopOffset
...
from iqpilot.selfdrive.controls.lib.helpers.green_follow_lead import GreenFollowLeadGate
from iqpilot.selfdrive.controls.lib.helpers.junction_hud import junction_hud_active, light_token
from iqpilot.selfdrive.controls.lib.helpers.turn_prep import UrbanTurnPrep
from iqpilot.selfdrive.controls.lib.helpers.nav_soft_curve import NavSoftCurveCap
from iqpilot.selfdrive.controls.lib.helpers.nav_lane_guide import NavLaneGuide
from iqpilot.iqlink.protocol import nav_turn_pending
...
class LongitudinalPlannerIQ:
  def __init__(self, CP: structs.CarParams, CP_IQ: structs.IQCarParams, mpc):
    ...
    self.nav_lane_guide = NavLaneGuide(params=self._params)
    self.nav_guide_hint = ""
  ...
  def _update_targets_id3(self, sm: messaging.SubMaster, v_ego: float, a_ego: float, v_cruise: float) -> tuple[float, float]:
    ...
    try:
      ...
      guide_v = self.nav_lane_guide.update(
        nav,
        engaged=bool(sm['selfdriveState'].enabled),
        iqlink_on=iqlink_on,
        v_ego=float(getattr(CS, "vEgo", v_ego)),
        posted_limit_ms=float(self.speed_limit_last or 0.0),
        slc_enabled=slc_apply_enabled,
      )
      self.nav_guide_hint = self.nav_lane_guide.hint
      if guide_v is not None:
        self.output_v_target = min(float(self.output_v_target), float(guide_v))
    except Exception:
      self.nav_guide_hint = ""
  ...
  def publish_longitudinal_plan_iq(self, sm: messaging.SubMaster, pm: messaging.PubMaster) -> None:
    ...
    def fill_plan(plan_msg) -> None:
      ...
      e2eAlerts.navGuideHint = str(self.nav_guide_hint or "")
