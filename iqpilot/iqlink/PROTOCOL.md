...
- **TBT types:** `lc*` → `fork` (HUD); true `_EXIT` (6/11) → `exit` (no left/right, so it does not auto-start NavExit). `0 < nTBTDist ≤ 150` + `turn_*` → `send_turn`; same distance window promotes near `lc_*` to `send_turn` (clears `send_lc`) unless `laneRecommend=straight`. `0 < nGoPosDist ≤ 150` → `arrive`, **only** stop `send_lc` / `send_turn`.
...
- **纵向**：变更驱动 + 粘限速；限速原值上报、执行侧常保底 60；红黄猛减速；显式 `remainS==1` 立刻起步（缺省/0 仍停）；APK green 停稳约 1 s 再走；右转待转仅 150 m 内免导航红停；BLE 断链不跟旧导航速度；IQ-link 开时关 NavExit/ALC；无绿波/SDI；车道 B 仅直行门控；`lc*` HUD 为 fork，**150 m 内**（非 straight）提升为 `send_turn`。
