...
- **纵向**：变更驱动 + 粘限速；限速原值上报、执行侧常保底 60；红黄猛减速；显式 `remainS==1` 立刻起步（缺省/0 仍停）；APK green 停稳约 1 s 再走；右转待转仅 150 m 内免导航红停；BLE 断链不跟旧导航速度；IQ-link 开时关 NavExit/ALC；无绿波/SDI；车道 B 仅直行门控；`lc*` 为 fork。
